...
async function getBitmaps(text, size, font) {
	const rl = readline.createInterface({
		input: fs0.createReadStream(`assets/${font}${size}.bdf`),
		crlfDelay: Infinity,
	});
	const $font = await $Font(rl[Symbol.asyncIterator]());
...
exports.generate = async function (text, size, options) {
	...
	const bitmaps = await getBitmaps(text, size, options.font);
	options.img = img;
